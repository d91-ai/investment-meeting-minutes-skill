...
- 音频+文稿输入时，正文以音频转录和 `aligned_transcript` 为主源，文稿只作为发言人、术语、遗漏和冲突的交叉对比材料；联网核验只确认实体、代码、术语和公开事实，不补写会议没有说过的内容。
...
- `standard`：普通文稿或音频+文稿。音频+文稿场景先完成音频转录并以 `aligned_transcript` 写正文，再用文稿交叉对比；随后批量本地标的候选查询，对命中风险的片段执行主流程自检和外部核验。
