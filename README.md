...
当前定位是 **main orchestrator + MAS process automation + deterministic validation**：所有会议纪要任务默认进入 MAS 编排，再根据长录音、噪音重、多人边界不清、多标的混杂、音频/文档冲突或高风险事实等具体风险增量选择 specialist subagents，不默认全量派发。最终 Markdown 和本地归档产物只由主流程统一生成；verification sidecar 仅作为内部审计文件，不作为正式交付物。
