...
- `重要信息修改记录`：仅在实体、代码、数字、单位、产品或术语发生可见实质纠正时出现，复用纠正时已有的依据，不启动第二轮查询。
...
7. 只核验会改变成稿文字的少量实体候选；本地批量查询后如仍需联网，所有未解决实体只合并召回一轮，不逐项追查。单一外部搜索结果不能证明模糊原音；证券代码不是正文强制字段。
...
- `scripts/validate_meeting_minutes_contract.py`：开发者按需使用的客观 Markdown 结构校验，不属于生产路径。
