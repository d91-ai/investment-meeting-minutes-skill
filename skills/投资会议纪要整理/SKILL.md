...
- MAS boundary: use MAS orchestration by default, following `references/mas_orchestration_contract.md`; select specialist agents only when their base or risk-specific artifact is required. For long transcripts, multi-speaker weekly meetings, or prior filler-retention failures, the main workflow must first build a hashed `speaker_turn_manifest` and let `speaker_editing_mode=auto` choose `skip` only for an explicitly reviewed, low-high-confidence-filler source; otherwise it uses deletion-only `full` editing. `skip` retains the manifest and routing evidence but creates no editor task or assembly receipt. `full` creates unique `speaker_turn_edit__...` artifacts; editors never assemble or write Markdown. The main workflow validates exact turn coverage and the deletion-only fidelity gate, assembles accepted edits by global `sequence`, and records `editing_assembly_receipt` before draft review. Other specialists may create `transcript_audit`, `source_reconciliation`, `entity_verification_report`, `target_attribution_review`, `fidelity_review`, and `export_manifest`, but they must not write or modify the final Markdown. Every specialist return must match the current dispatch `run_id`, `task_id`, phase, owner, and allowed artifact set.
...
For a long or filler-heavy source, run `scripts/build_speaker_turn_manifest.py` on the selected unedited body source and pass the resulting JSON to `build_mas_task_bundle.py --speaker-turn-manifest`. Keep `speaker_editing_mode=auto` unless the main workflow has current-session evidence for an explicit `skip` or `full` override. Check `bundle.speaker_editing`: an auto `skip` is valid only when a reviewed-source marker and low high-confidence-filler density both pass. For `full`, use `plan_mas_next_action.py --max-parallel N`: the plan balances at most six prompts into each agent batch and groups at most `N` agent batches into one dispatch wave; every prompt still returns its own task-bound JSON. After every edit artifact has been ingested, run `scripts/assemble_speaker_turn_edits.py`; do not enter `draft_review` until the collector accepts the main-owned assembly receipt. Never regroup the assembled body by speaker: repeated appearances remain at their original global sequence positions.
...
- `build_speaker_turn_manifest.py`: split the selected unedited transcript into hashed, source-ordered turns and per-speaker shards without deleting filler words first.
...
- `assemble_speaker_turn_edits.py`: validate complete per-shard editor returns, assemble the main-owned working body by global turn sequence, and bind it with `editing_assembly_receipt`.
