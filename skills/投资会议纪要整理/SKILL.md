...
`## 一、发言整理` 只能包含源材料中的发言以及必要的正常编辑。正常编辑可以清理纯寒暄、无信息口头填充、机械重复和错误断句，但必须保留否定、自我纠正、观点强度、条件、时间、数字和实质信息；不得机械删除连续出现的否定词。不得写入 AI 判断、解释、总结、推论、补充背景或处理说明。已确认的转写修复、公司名、股票代码和规范化内容使用普通字体；只有仍无法确认的最小源材料片段留在原句原位置并加粗。AI 核验内容只能进入 `doubtful_items`、verification sidecar、MAS artifact 和最终存疑表的审核列。
