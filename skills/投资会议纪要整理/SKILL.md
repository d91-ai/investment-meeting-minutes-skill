...
- MAS evidence boundary: `external_evidence_paths` may contain only an `http(s)` URL or an opaque public source ID, never a local candidate path. When `export_manifest.known_unverified_parts` is non-empty, the declared verification sidecar must exist and pass the shared sidecar validator before collection can succeed.
...
- Validators: keep validation to encoding, Markdown structure, and regression samples. Do not add content-direction validators or semantic-consistency hard checks.
