...
- 子任务只标记少量可能未决或发生重要纠正的原始片段，不作跨包最终结案。主流程合并后只处理接缝、统一排版和这些已定位项目，不再全文重写。
...
3. 汇总少量候选，按 `verification_policy.md` 使用已读的本场证据统一结案；真正未决项进入存疑，重要实质纠正进入修改记录，不再查询。
4. 只重试失败的包；只有已定位的跨包风险才回看相应片段及最小相邻上下文。

不增加全文预标注副本、固定比例优化、候选状态机、第二套转写、全文重写或独立语义审查。
