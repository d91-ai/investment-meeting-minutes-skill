...
- 分段内只能标记可能存疑的原始片段，不能依据局部上下文直接生成最终存疑表。
...
4. 整个来源轮次被删除时，对空的 `reference_segments` 或 `minutes_segments` 提供对应 omission reason。不接受旧版 `reference_text` / `minutes_text`。
...
7. 主流程汇总少量候选存疑，逐项检查其在整场会议中的全部出现位置和相关上下文，再对仍未解决的公开身份问题做定向查询。只有查询后仍不唯一或来源确有冲突时才进入最终存疑表。
