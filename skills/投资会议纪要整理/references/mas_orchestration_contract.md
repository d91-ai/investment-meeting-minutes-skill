...
输入：
- 当前会议全文、已确认实体、实体候选、本地代码候选、外部核验路径。
...
- 先复用全文已确认实体，再以别名、完整拼音、拼音首字母和近似拼音补充候选；音近信号只用于召回，必须结合局部语境、相邻实体和外部证据筛选。
...
- 除非用户明确指定材料为已核定终稿或仅作标准参考，是否将所有输入按未经完整校对的原始材料重新审核；不得因文件名、既有排版、粗体、存疑表或整理痕迹而继承旧结论或降低纠错、语言清理、实体核验和逐句保真强度。
...
- 按基础 Skill 清理纯寒暄、无信息填充、机械重复和错误断句，同时保留否定、自我纠正、观点强度、条件、时间、数字和实质信息。
...
- `draft_review`: run only after the main workflow has completed both output layers, entity/code handling, inline doubtful mapping, and role-relevant source spans. Before dispatch, run `prepare_draft_review.py` and freeze the exact Markdown path and SHA-256 from a passing readiness receipt. Typical tasks: target attribution and fidelity review.
...
- Tasks in the same phase may run in parallel; tasks across phases must wait for their prerequisites. `plan_mas_next_action.py` exposes non-editing tasks in `parallel_dispatch_groups`; dispatch every member of one `draft_review` group concurrently against the same passing readiness receipt. This changes scheduling only and never removes a role, check, artifact, or re-review requirement.
...
- Immediately before the first semantic-review dispatch for a draft, run `scripts/prepare_draft_review.py NOTE.md --out "$MAS_DISPATCH/draft_review_readiness.json" --json`. A failed readiness result blocks dispatch. Do not edit the Markdown between this check and the parallel review wave; reviewers must report the exact receipt path and SHA-256. The receipt is a scheduling aid, not a new MAS artifact and not a substitute for either semantic review.
...
- Every `target_attribution_review` and `fidelity_review` must bind to the same current Markdown path and SHA-256. If either review reports an actionable finding, revise the Markdown and repeat both affected semantic reviews on the new bytes after rerunning readiness. A receipt that merely claims the old findings were applied cannot substitute for a fresh clean review. Missing, stale, failed, zero-scope, or still-actionable review artifacts block `final_verification` and export. Formal export must use `export_to_obsidian.py --mas-summary "$MAS_DISPATCH/mas_run_summary.json"`; the exporter reruns the deterministic validators and accepts only a completed `draft_review` gate bound to the exact Markdown bytes. The post-export `export_manifest` and `final_verification` remain required before user-facing delivery.
